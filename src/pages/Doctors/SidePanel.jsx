...
const SidePanel = ({ doctorId, ticketPrice, timeSlots }) => {
  ...
  const confirmBooking = () => {
    // Here you would normally send the appointment data to the backend for storage
    // Example: axios.post(`${BASE_URL}/appointments`, { doctorId, selectedDate, ... });
    setAppointment(selectedDate);
    setShowModal(false);
  };
        ...
        <ul className="space-y-2">
          {timeSlots?.map((item, index) => (
            ...
            >
              ...
              <p className="text-sm font-medium text-gray-700">
                {/* This can be adjusted as per the format needed */}
              </p>
            </li>
          ))}
        </ul>
